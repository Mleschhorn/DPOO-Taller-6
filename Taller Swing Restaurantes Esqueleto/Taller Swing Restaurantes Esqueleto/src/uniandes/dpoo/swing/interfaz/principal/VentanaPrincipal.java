...
public class VentanaPrincipal extends JFrame
{
    ...
    public void mostrarVentanaMapa( )
    {
    	...
    	if (ventanaMapa == null || !ventanaMapa.isVisible()) {
            List<Restaurante> todosRestaurantes = getRestaurantes(true); // Obtén todos los restaurantes
            ventanaMapa = new VentanaMapa(this, todosRestaurantes); // Crea la ventana del mapa
            ventanaMapa.setVisible(true); // Muestra la ventana
        }
    ...
    public void agregarRestaurante( String nombre, int calificacion, int x, int y, boolean visitado )
    {
        ...
        // Agrega el restaurante al diario
        mundo.agregarRestaurante(nuevoRestaurante);
        
        // Actualiza la lista de restaurantes mostrados en la interfaz
        actualizarRestaurantes();
    }
    ...
    private void actualizarRestaurantes( )
    {
        ...
        pLista.actualizarRestaurantes(todos); // Actualiza la lista de restaurantes en el panel
        
        // Si hay restaurantes, selecciona el primero por defecto
        if (!todos.isEmpty()) {
            cambiarRestauranteSeleccionado(todos.get(0));
        } else {
            // Si no hay restaurantes, limpia los detalles mostrados
